...
recipe.bindAdjuster = function() {
	recipe.rootNode.innerHTML = recipe.rootNode.innerHTML.replace(/(donne )(\d+)/i,
			function(unused_match, serves, quantity, what) {
		return serves + '<input type="number" min="3" value="' + quantity + '" step="3"' + quantity/3 + '" onchange="recipe.adjustIngredients(this.value/' + quantity + ')" class="control" />'
	});
};
...
recipe.Ingredient.prototype.pluralize = function() {
	if (this.isSingular() || recipe.Ingredient.INVARIABLE.includes(this.unit)) {
	    return this.unit ;
	}
	else {
		return this.unit + 's';
...
recipe.Ingredient.INVARIABLE = ['ml', 'cu'];})();
